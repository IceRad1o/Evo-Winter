...
public class ItemsTable : ExUnitySingleton<ItemsTable>
{
    public AudioClip[] pickUpSounds;
    [System.Serializable]
    public struct ItemTrim
    {
        public CharacterSkin.Type parentNode;
    ...
    public struct ItemData
    {
        ...
        /// </summary>
        public ItemDroping droping;
        public Sprite sprite;
        //trims the item will give to item's owner;
        public ItemTrim[] trims;

        public ItemPickUpSound pickUpSound;
        public int buffID;
    ...
    /// <summary>
    /// ItemData数组,方便在编辑器中添加信息和遍历
    /// </summary>
    public ItemData[] itemDataArray;
    /// <summary>
    /// 道具信息字典,以便快速查找特定道具
    /// </summary>
    private Dictionary<int, ItemData> itemDataDict = new Dictionary<int, ItemData>(100);
    ...
    public int GetItemType(int ID)
    {
        ...
        return (int)FindItemsByID(ID).type;
    ...
    public int GetItemDroping(int ID)
    {
        ...
        return (int)FindItemsByID(ID).droping;
    ...
    /// <param name="ID">道具ID</param>
    /// <returns>item的SkillID</returns>
    public ItemQuality GetItemQuality(int ID)
    {
    ...
    */
    public int[] GetItemsByType(bool includeingImm = false, bool includeingDis = false, bool includeingIni = false)
    {
        ...
        {
            if (itemDataArray[i].type == ItemType.Immediately && includeingImm)
                itemsIDs.Add(itemDataArray[i].ID);
            if (itemDataArray[i].type == ItemType.Disposable && includeingDis)
                itemsIDs.Add(itemDataArray[i].ID);
            if (itemDataArray[i].type == ItemType.Initiative && includeingIni)
                itemsIDs.Add(itemDataArray[i].ID);
        }
    ...
    /// <param name="bossDroping"></param>
    /// <param name="boxDroping"></param>
    /// <returns></returns>
    public int[] GetItemsByDoping(bool roomDroping = false, bool bossDroping = false, bool boxDroping = false)
    {

        List<int> itemsIDs = new List<int>();
        for (int i = 0; i < itemDataArray.Length; i++)
        {
            int drop = (int)itemDataArray[i].droping;
            if ((drop== 1 || drop== 3 || drop== 5 || drop== 6) && roomDroping)
                itemsIDs.Add(itemDataArray[i].ID);
            if ((drop== 0 || drop== 3 || drop== 4 || drop== 6) && bossDroping)
                itemsIDs.Add(itemDataArray[i].ID);
            if ((drop== 2 || drop== 4 || drop== 5 || drop== 6) && boxDroping)
                itemsIDs.Add(itemDataArray[i].ID);
        }
    ...
    public ItemData FindItemsByID(int ID)
    {
    ...
    public ItemsTable()
    {
        ...
        //item.quality = "C";
        //item.itemName = "生命之精";
        //item.itemIntro = "拥有浓郁的生命活力，使用后恢复一格体力";
        //itemsData.Add(item);
    ...
    private void InitDict()
    {
       for(int i=0;i<itemDataArray.Length;i++)
        {
            Debug.Log("I:" + i);
            itemDataDict.Add(itemDataArray[i].ID, itemDataArray[i]);
        }
    }
}
