...
public enum RmType { Hidden = -3, Boss, Start, Non, Box, Shop, Altar, Normal,Story,Test,HardLevel,Organ};
[Serializable]
//单个房间信息
...
public class RoomManager : ExUnitySingleton<RoomManager>
{
    ...
    static Vector3 roomLeftTop= new Vector3(-6.6f, 1f, 1f);
    static Vector3 roomRightTop = new Vector3(6.6f, 1f, 1f);
    static Vector3 roomLeftBottom = new Vector3(-6.6f,-9f, 0);
    static Vector3 roomRightBottom = new Vector3(-6.6f, 2.2f, 0);
    static Vector3 roomCenter = new Vector3(-6.6f, 2.2f, 0);
    ...
    private Vector3[] doorPos = { new Vector3(0f, 1.4f * 2f, 0f), new Vector3(0f, -4.7f * 2f, 0f), new Vector3(-6.8f, -1.46f * 2f, 0f), new Vector3(6.8f, -1.46f * 2f, 0f) };
    ...
    //随机布局小怪
    void LayoutCharacterAtRandom(RoomElement[] objectArray, int minimum, int maximum)
    {
        ...
        for (int i = 0; i < objectCount; i++)
        {
            ...
            if (RoomType == (int)RmType.Normal || RoomType == (int)RmType.Boss)
            {
                ...
                ChoiceID = objectArray[Random.Range(0, objectArray.Length)].RoomElementID;     
                roomInfo.roomElementInfoList.Add(new RoomElementInfo(ChoiceID, randomPosition));
    ...
    public void SetupPresetRoom(RoomInfo roomdata,bool byRmType=true,int presetID=0)
    {
        roomInfo = roomdata;

        if (byRmType)
        {
            var maps=CombinationTable.mapTypeDict[roomInfo.type];
            if (maps.Length==0)
            {
                SetupRoom(roomdata);
                return;
            ...
            else
            {
                ...
                maps[Random.Range(0, maps.Length)].AddRECombinationInfoToList(roomdata.roomElementInfoList);
            }
        }
        else
            CombinationTable.mapIDDict[presetID].AddRECombinationInfoToList(roomdata.roomElementInfoList);
    ...
    public void UpdateREList()
    {
        ...
        {
            if (list[i].CompareTag("Player")|| list[i].CompareTag("Friend"))
                continue;
    ...
    public void SetupRoom(RoomInfo roomdata)
    {
        ...
        InitPositionList();
        //LayoutWall();
        LayoutCharacterAtRandom(CharacterTable.Instance.characters, minEnemyNumber, maxEnemyNumber);
    ...
    public void LoadRoom(RoomInfo Info)
    {
        ...
        for (int i = 0; i < REList.Count; i++)
        {
            ...
            else if (id >= REID.Character&& id < REID.CharacterEnd)
            {
                //生成Character
                Debug.Log("id:" + id);
                ins = Instantiate(CharacterTable.IDDict[id], REList[i].Position, CharacterTable.IDDict[id].transform.rotation) as RoomElement;
            }
            else
            {
                //Debug.Log("id:" + id);
