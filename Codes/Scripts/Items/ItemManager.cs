...
public class ItemManager : ExUnitySingleton<ItemManager>
{
    ...
    public Item CreateItemID(int ID,Vector3 pos=default(Vector3),int state=0,bool trans = true)
    {
        ...
        {
            itemInstance = Instantiate(itemsDisposable, pos,Quaternion.identity) as DisposableItem;

            itemInstance.Create(itemID);
        }
        else if (ItemsTable.GetItemType(itemID) == 0)
        {
            itemInstance = Instantiate(itemImmediately, pos, Quaternion.identity) as ImmediatelyItem;
            itemInstance.Create(itemID);
      

        }
        else if (ItemsTable.GetItemType(itemID) == 2)
        {
            itemInstance = Instantiate(itemInitiative, pos, Quaternion.identity) as InitiativeItem;
            itemInstance.Create(itemID);
        
        }
        itemInstance.transform.SetParent(RoomManager.Instance.ground);
        return itemInstance;
    }
